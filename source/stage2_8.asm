...
MAX_STAGE_DWORDS  equ 256        ; per STAGE_WRITE_BLOCK call, matches
                                  ; READ_BLOCK's own sizing (1024 bytes,
                                  ; safely under the Ethernet MTU)
MAX_WRITE_SECTORS equ 128        ; per DISK_WRITE_SECTORS commit call =
                                  ; 64KB = exactly STAGE_BUF's size --
                                  ; conservative, widely-supported single-
                                  ; BIOS-call size for extended INT13h writes
